...
def get_result(data):
    if data['task'] == 'get_flight_offers':
        ...
        flight_data = amedeus_client.getting_flight_offers(origin=origin, destination=destination, departure_date=departure_date, n_adults=n_adults)
        return flight_data
    else:
        city_code = data['params']['city_code']
        hotel_data = amedeus_client.getting_hotels(city_code=city_code)
        return hotel_data
